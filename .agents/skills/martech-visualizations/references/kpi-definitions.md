...
- **Date window:** `customer_created_date < DATE_FORMAT(DATE_TRUNC('MONTH', CURRENT_DATE()), 'yyyy-MM')` — current month excluded; `customer_created_date` is STRING `'YYYY-MM'`, compare as string (`Observed SQL`); widgets also filter year(s). Dynamic snapshot suffix = previous calendar month (`yyyy_MM`).
...
- **ROI fields** — `roi_12m/24m/60m` = GP ÷ `acq_spend` without subtracting 1. If surfaced, document this exact definition — not textbook ROI, not ROAS. Upstream `roi_12m` NULL-when-zero-spend behavior: `Documented` (NGR V1 Pipeline Notion page, Algorithm Step 2: "returns NULL if acquisition spend is zero"); no dashboard-source citation for the table-level behavior.
